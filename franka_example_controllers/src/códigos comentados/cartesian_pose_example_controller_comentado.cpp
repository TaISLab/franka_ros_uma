...
#include <franka_example_controllers/cartesian_pose_example_controller.h> // incluye el controlador
...
bool CartesianPoseExampleController::init(hardware_interface::RobotHW* robot_hardware,
  ...
  // obtiene la interfaz de pose cartesiana de Franka
  cartesian_pose_interface_ = robot_hardware->get<franka_hw::FrankaPoseCartesianInterface>();
  if (cartesian_pose_interface_ == nullptr) { // nullptr: puntero nulo. 
    ROS_ERROR(
  ...
  std::string arm_id; // obtiene el ID del brazo
...
void CartesianPoseExampleController::update(const ros::Time& /* time */,
  ...
  new_pose[12] -= delta_x;
  new_pose[14] -= delta_z;
  cartesian_pose_handle_->setCommand(new_pose); // Establece el comando de pose cartesiana
}
